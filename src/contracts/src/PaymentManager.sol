...
contract PaymentManager is ReentrancyGuard {
    ...
    address public royaltyManager; // New royalty manager for distribution
    ...
    uint256 public constant PLATFORM_FEE = 350;            // 3.5% on withdrawal
    ...
    /**
     * @notice Sets the RoyaltyManager contract address
     * @param _royaltyManager Address of the RoyaltyManager contract
     */
    function setRoyaltyManager(address _royaltyManager) external {
        require(royaltyManager == address(0), "Already set");
     ...
     * Now delegates to RoyaltyManager for consistent distribution logic
    ...
    function distributeRevenue(
        ...
        // If RoyaltyManager is set, delegate to it for consistent distribution logic
        if (royaltyManager != address(0)) {
            (bool success,) = royaltyManager.call{value: msg.value}(
                abi.encodeWithSignature(
                    "distributeAuthorizationRoyalty(uint256,address,address[])",
                    workId,
                    directCreator,
                    ancestors
                )
        ...
        // Fallback to original logic if RoyaltyManager not set
