...
// 获取当前网络配置
export function getNetworkConfig(chainId = 7001) {
  return CONTRACTS[chainId] || CONTRACTS[7001];
}

// 获取合约地址
export function getContractAddress(contractName, chainId = 7001) {
  const config = getNetworkConfig(chainId);
  return config.contracts[contractName];
}

// 获取所有合约地址
export function getAllContractAddresses(chainId = 7001) {
  const config = getNetworkConfig(chainId);
