...
import { useWeb3, useTransaction } from '../hooks/useWeb3';
...
export function Web3Provider({ children }) {
  ...
  const fetchUserWorks = async (address) => {
    ...
    try {
      const response = await blockchainAPI.getUserWorks(address);
      dispatch({
  ...
  const fetchMarketplaceListings = async () => {
    ...
    try {
      const response = await blockchainAPI.getMarketplaceListings();
      dispatch({
